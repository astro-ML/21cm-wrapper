...
class Probability:
    def __init__(self, prior_ranges: dict, z_chunks: int|list[int], bins: int, debug: bool = True, 
...
class Simulation(Leaf):
    def __init__(self, Probability: Probability, redshift: float, data_path: str = "./mcmc_data/", noise_type: tuple = None,
        ...
        if (len(fid_file) != 0) and not regenerate_fiducial:
            ...
            fiducial_cone = self.load(self.data_path + "fiducial_cone.h5", lightcone=True)
            ...
            self.userparams.update(N_THREADS = os.cpu_count() if os.cpu_count()<33 else 32)
            fiducial_cone = self.run_lightcone(redshift=redshift, save=False, 
                                                random_seed=42, filter_peculiar=False, sanity_check=True)
            self.userparams.update(N_THREADS = temp_threads)
            self.save(obj=fiducial_cone, fname="fiducial_cone", direc=data_path, run_id="")
        ...
        if (len(ps_file) != 0) and not regenerate_fiducial:
            ...
            if debug: print("PS is ", self.fiducial_ps)
        else:
    ...
    def step(self, parameters: list[float]) -> float:
        ...
        # run lightcone sim
        test_lc = self.run_lightcone(redshift=self.redshift, **parameters,
                           sanity_check=True, filter_peculiar=False, save=False)
        if self.noise is not None:
            if self.noise[0] == 1:
                ...
                #test_lc.brightness_temp = Simulation.gaussian_noise(test_lc.brightness_temp, **self.noise[1:])
            else: print("Noise-type not found you gave: ", self.noise)
        # compute probability
...
class Flower(Simulation):
    def __init__(self, Probability: Probability, data_path: str = "./mcmc_data/", noise_type: tuple = None, 
                 debug: bool = False, regenerate_fiducial: bool = True, redshift: float = 5.5, fid_params: dict = {}):
        '''
        ...
        super().__init__(Probability = Probability, redshift = redshift, data_path = data_path, 
                         noise_type=noise_type, debug = debug, regenerate_fiducial = regenerate_fiducial, 
                         **fid_params)
        self.Prob = Probability
        
    def run_emcee(self, filename: str = "./results_emcee.h5", threads: int = 1, walkers: int = 12, nsteps: int = 1000) -> None:
        ...
        with schwimmhalle as p:
            sampler = emcee.EnsembleSampler(nwalkers=walkers, ndim=ndim, 
                                            log_prob_fn = self.step, pool=p, backend=backend)
    ...
    def initialize_parameter(self, shape: tuple[int,int]):
        ...
        for i in range(shape[0]):
            initial_params[i,:] = self.extract_values(self.generate_range(self.Prob.prior_ranges, self.uniform))
